...
Validations.prototype.validateAddProduct = function() 
{
    ...
    if (!(this.isPNG(image) || this.isJPG(image))) 
    {
...
Validations.prototype.validateAddArticle = function() 
{
    ...
    if (!(this.isPNG(image_path) || this.isJPG(image_path)))
    {
        $(".error[for=image]", ".article-add-form").fadeIn("slow");
        errors++;
    } else {
...
Validations.prototype.validateAddGallery = function() 
{
    ...
    var $error_images = $(".error[for=images]", ".gallery-add-form");
    var gallery_name = $("input[name=name]", ".gallery-add-form").val().toString(); // toString -a la segura-
    var number_of_images = $("img.gallery-image").length;
    var errors = 0;
...
Validations.prototype.validateAddMembers = function() 
{
    ...
    {
        $error_name.text("Ingrese un nombre. Max 50 caracteres. Actualmente tienes " + profile_name.length);
        $error_name.fadeIn("slow");
        errors++;
    }
    else
    ...
    if(profile_image === "")
    {
        $error_profile_image.fadeIn("slow");
        errors++;
    }
    else
    {
        $error_profile_image.fadeOut();
    }
