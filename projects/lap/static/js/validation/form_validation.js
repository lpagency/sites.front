...
var expr_url = /(http|https):\/\/[\w-]+(\.[\w-]+)+([\w.,@?^=%&amp;:\/~+#-]*[\w@?^=%&amp;\/~+#-])?/; //expression for a url
 ...
 * @return {Boolean}     true if the format fits with an URL
...
Validations.prototype.validateAddArticle = function() 
{
    ...
    var article_text = $("textarea[name=text]", ".article-add-form").html();
    ...
    // @todo : image, should be png or jpg
    if (!this.isPNG(image_path))
    {
        $(".error[for=image]", ".article-add-form").fadeIn("slow");
        errors++;
    } else {
...
Validations.prototype.validateAddBanner = function() 
{
    ...
    // @todo : should be valid png and jpg
    if (!this.isPNG(image))
    {
        $error_image.fadeIn("slow");
        errors++;
...
Validations.prototype.validateAddEvent = function() 
{
    ...
    var description = $("textarea[name=description]", ".event-add-form").text();
...
$(document).ready(function() {
    ...
    {         
        gallery_name = $("#gallery-name-field").val().toString(); // toString -a la segura-
        number_of_images = $("img.gallery-image").length;
        errors = 0;     
        if(gallery_name == ""){
            $("#error_gallery_name").fadeIn("slow");
            errors++;
        }else{
            $("#error_gallery_name").fadeOut();
        }
        if(number_of_images == 0){
            $("#error_gallery_images").fadeIn("slow");
            errors++;
        }else{
    ...
    $(document).on("click", "#submit_profile_form", function(evt)
    {
        ...
        profile_phone = $("#profile_phone_field").val().toString();
        profile_url = $("#profile_url_field").val().toString();
        errors = 0;

        if(!validations.isURL(profile_url)){
            $("#error_profile_url").fadeIn("slow");
            errors++;
        }else{
            $("#error_profile_url").fadeOut();
        }        
        if(!expr_numbers.test(profile_phone)){
        ...
        if($("#profile_image_holder1").attr("src") == "/me/static/img/0013.png"){
            $("#error_profile_photo1").fadeIn("slow");
            errors++;
        }else{
            $("#error_profile_photo1").fadeOut();
        }
        if($("#profile_image_holder2").attr("src") == "/me/static/img/0013.png"){
            $("#error_profile_photo2").fadeIn("slow");
            errors++;
        }else{
